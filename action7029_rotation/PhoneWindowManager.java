...
public class PhoneWindowManager implements WindowManagerPolicy {
	...
	private static final String PROPERTY_HDMI_PLUG_IN = "persist.sys.hdmi.plug.in";
    /**
    ...
    static final String BAR_STYLE_4_1 = "tablet";
    static final String BAR_STYLE_4_2 = "phone";
    static final String mBarStyle = SystemProperties.get("ro.systemui.bar_style", BAR_STYLE_4_2);
    ...
    // ywwang: added for notebook
    int mNotebookRotation;
    boolean mNotebookRotationLock;
    ...
    int mCurrentOrientation = Surface.ROTATION_0;//yangheyao add for rotate
	...
	long mLastShortPower = SystemClock.uptimeMillis();
    ...
    class MyOrientationListener extends WindowOrientationListener {
	    ...
	    Slog.d(TAG, " yangheyao.....settotation");
            updateRotation(false);
        }
    }
    ...
    private void interceptPowerKeyDown(boolean handled) {
        ...
        if (!handled) {
            //modified by zzzhou for ebox
            mHandler.postDelayed(mPowerLongPress, ViewConfiguration.getGlobalActionKeyTimeout());
			//mHandler.postDelayed(mPowerLongPress, 0);
        }
    }
    ...
    public void setInitialDisplaySize(Display display, int width, int height, int density) {
        ...
        if (shortSizeDp < 600) {
            ...
            if(mBarStyle.equals(BAR_STYLE_4_1)){
                mHasSystemNavBar = true;
                mNavigationBarCanMove = false;
            }
        }

        //allow system property to force tablet UI
        if( "1".equals(SystemProperties.get("ro.config.forcetabletui", "0"))){
         //   if(SystemProperties.get("ro.build.version.sdk").equals("16"))
         //   {
                mHasSystemNavBar = true;
                mNavigationBarCanMove = false;
         //   }
        ...
        //ywwang for notebook pc ,lock rotation
        mNotebookRotationLock = SystemProperties.getBoolean("system.display.nbrotationlock", false);
        if ("portrait".equals(SystemProperties.get("system.display.nbrotation"))) {
            mNotebookRotation = mPortraitRotation;
        } else {
            mNotebookRotation = mLandscapeRotation;
        }
    }
    private void resetSettingsRotationInternal() {
        ContentResolver resolver = mContext.getContentResolver();
        synchronized (mLock) {
    	    int userRotationMode = Settings.System.getIntForUser(resolver,
                    Settings.System.ACCELEROMETER_ROTATION, 0, UserHandle.USER_CURRENT) != 0 ?
                            WindowManagerPolicy.USER_ROTATION_FREE : WindowManagerPolicy.USER_ROTATION_LOCKED;
            int userRotation = Settings.System.getIntForUser(resolver,
                    Settings.System.USER_ROTATION, Surface.ROTATION_0,
                    UserHandle.USER_CURRENT);
                    
            if(userRotationMode == WindowManagerPolicy.USER_ROTATION_LOCKED && userRotation != mDefaultRotation){         	
    			Settings.System.putIntForUser(resolver, Settings.System.USER_ROTATION,mDefaultRotation,UserHandle.USER_CURRENT);
    	    }
    	}
    }
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
		...
		//yangheyao add for rotate start
	else if (keyCode == KeyEvent.KEYCODE_F6) {
            if (down) {
                //DSJ ADD FOR ROTATE
                WindowManager wm = (WindowManager)mContext.getSystemService(Context.WINDOW_SERVICE);
                int mCurrentRotation = wm.getDefaultDisplay().getRotation();
                Slog.d(TAG, " yangheyao.....up" + mCurrentRotation);
               ...
               case Surface.ROTATION_180:
                ...
                mOrientationListener.onProposedRotationChanged(mCurrentOrientation);
		//setRotationLw(mCurrentOrientation);
                Slog.d(TAG, " yangheyao.....down"+mCurrentOrientation);


            }
            return -1;
        }else{}
        //yangheyao add for rotate end
        ...
        public void onInputEvent(InputEvent event) {
            ...
            try {
                if (event instanceof MotionEvent
                        && (event.getSource() & InputDevice.SOURCE_CLASS_POINTER) != 0) {
                    final MotionEvent motionEvent = (MotionEvent)event;
                    int keytype= mPointerLocationMode==0 ? MotionEvent.ACTION_DOWN: MotionEvent.ACTION_UP;
                    if (motionEvent.getAction() == keytype) {
    ...
    void setHdmiPlugged(boolean plugged) {
        if (mHdmiPlugged != plugged) {
            ...
            SystemProperties.set(PROPERTY_HDMI_PLUG_IN, plugged ? "1" : "0");
    ...
    void initializeHdmiState() {
        ...
        if (new File("/sys/devices/virtual/switch/hdmi/state").exists()) {
            mHDMIObserver.startObserving("DEVPATH=/devices/virtual/switch/hdmi_plug");

            final String filename = "/sys/class/switch/hdmi_plug/state";
    ...
    void handleVolumeKey(int stream, int keycode) {
        ...
        try {
            ...
            /*
             * audioService.adjustStreamVolume(stream, keycode ==
             * KeyEvent.KEYCODE_VOLUME_UP ? AudioManager.ADJUST_RAISE :
             * AudioManager.ADJUST_LOWER, 0);
             */
            // added by zzzhou to handle mute events
            if (keycode == KeyEvent.KEYCODE_VOLUME_MUTE) {
                audioService.setStreamMute(stream, audioService.isStreamMute(stream) ? false : true, null);
                audioService.adjustStreamVolume(stream, AudioManager.ADJUST_SAME, AudioManager.FLAG_SHOW_UI);
            } else {
                audioService.adjustStreamVolume(stream, keycode == KeyEvent.KEYCODE_VOLUME_UP ? AudioManager.ADJUST_RAISE
                        : AudioManager.ADJUST_LOWER, 0);
            }
        } catch (RemoteException e) {
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags, boolean isScreenOn) {
        ...
        case KeyEvent.KEYCODE_CAPTURE:
            ...
            case KeyEvent.KEYCODE_VOLUME_MUTE: {
                ...
                } else { // added by zzzhou,handle mute events here, not pass to
                         // applications
                    result &= ~ACTION_PASS_TO_USER;
                    if (down)
            ...
            case KeyEvent.KEYCODE_POWER: {
                ...
                if (down) {
                    ...
                    cancelPendingScreenshotChordAction();
                    if (interceptPowerKeyUp(canceled || mPendingPowerKeyUpCanceled) && (SystemClock.uptimeMillis() > mLastShortPower)) {
                        // zzzhou:start
                        mLastShortPower = SystemClock.uptimeMillis() + 2000;
                        goHome();
                        if (isMusicActive()) {
                            killMediaserver();
                        }
    ...
    private void killMediaserver() {
        ...
        formats[0] = Process.PROC_OUT_STRING;
        for (int i : pids) {
            Process.readProcFile("/proc/" + i + "/status", formats, strs, null, null);
            for (String str : strs) {
                if (str != null && (str.contains("mediaserver")
                /* || str.contains("avahi-daemon") */)) {
                    Log.d(TAG, "Got mediaserver process, pid = " + i + ", kill it!");
    ...
    public void enableKeyguard(boolean enabled) {
        if (mKeyguardMediator != null) {
            //added by fubingrui set keyguard disabled
            //mKeyguardMediator.setKeyguardEnabled(enabled);
            mKeyguardMediator.setKeyguardEnabled(false);
        }
    }
...
/*        if (false) {
        ...
        synchronized (mLock) {
            ...
            }else if(mNotebookRotationLock){
              preferredRotation = mNotebookRotation;
	...
	Slog.d(TAG,"yangheyao ---return");//yangheyao add for rotate
	return mCurrentOrientation;//yangheyao add for rotate
    }
